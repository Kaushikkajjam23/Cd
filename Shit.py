...
follow_sets = {}
for nt in example_grammar:
    follow(grammar=example_grammar, symbol=nt, start_symbol='S', first_sets=first_sets, follow_sets=follow_sets)
...
def construct_parsing_table(grammar, first_sets, follow_sets):
    ...
    for non_terminal, productions in grammar.items():
        for production in productions:
            first_set = first(grammar, production[0], first_sets)
            for terminal in first_set:
                if terminal != 'ε':
                    parsing_table[(non_terminal, terminal)] = production
            if 'ε' in first_set:
                follow_set = follow_sets[non_terminal]
...
# Print parsing table
print("\nParsing Table:")
print("{:<10} {:<10} {:<10}".format("Non-term", "Terminal", "Production"))
for key, value in parsing_table.items():
    print("{:<10} {:<10} {:<10}".format(key[0], key[1], value))
